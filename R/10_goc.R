...
Svy.Min = suppressWarnings({
  zoo::rollapply(Svy.Min, width=2*svy.scope()+1, partial=1, FUN=min, na.rm=TRUE)
})
...
Svy.Max = suppressWarnings({
  zoo::rollapply(Svy.Max, width=2*svy.scope()+1, partial=1, FUN=max, na.rm=TRUE)
})
...
GoC = YV.int(1)
GoC.Expl = YV.char("GoC=No accepted empirical data")
...
GoC.Expl[cbind(index$Y, index$V)] =
