...
wuenic.survey = function(Survey, Decisions)
{
  ...
  index = which(tround(Ana[, V13, , drop=FALSE]) != H3Adj, arr.ind=TRUE)
  ...
  Info = array("", dim=c(length(Yn()), length(Vn()), length(Idn)),
                   dimnames=list(Y=Yn(), V=Vn(), Id=Idn))
  if(any(index))
  {
  ...
  # Svy.Cov = YV.int()
  Cov = tround(apply(Accept, c(1, 2), mean, na.rm=TRUE))
  
  Expl = YV.char()
  Expl[] = sprintf(
    "Survey evidence of %g percent based on %i survey(s). ", 
    Cov, apply(!is.na(Accept), c(1, 2), sum))
