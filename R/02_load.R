...
#'    AND vaccine IN vaxs AND type = "admin"
#'
admin = function(mdb, ccode)
{
  # Check: Why is list of vaccines limited to vaxs?
...
#' 
#' @return
#' Year by Vaccine table with target numbers
#' 
#' @details
#' SELECT vaccine, annum, reportedDenom, reportedNum, coverage
#'   FROM REPORTED_NUMERATOR_DENOMINATOR
...
#'
surviving = function(mdb, ccode)
{
  t = mdb_get(mdb=mdb, tab="DEMOGRAPHIC")
  t$country = toupper(t$country)
  t = t[t$country == toupper(ccode) & t$annum >= 1997 &
    t$Indicator == "SI" & t$Valor > 0, c("annum", "Valor")]
