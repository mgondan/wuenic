# 6. Integrate estimates from reported data and surveys and define the
# according anchor points (reported | survey | wgd).

Anchor.Rule = matrix(NA_character_, 
  nrow=length(Yn), ncol=length(Vn), dimnames=list(Y=Yn, V=Vn))
Anchor.Info = 
  matrix(NA_character_, nrow=length(Yn), ncol=length(Vn), dimnames=list(Y=Yn, V=Vn))
Anchor.Cov = 
  matrix(NA_integer_, nrow=length(Yn), ncol=length(Vn), dimnames=list(Y=Yn, V=Vn))
